...
void swap(struct node* ref1, struct node* ref2) {
    ...
    if (ref1 == NULL || ref2 == NULL)
        fprintf(stderr, "swap() :: No node to swap.\n");

    struct node* tmp = ref1 -> next;

    ref1 -> next = ref2 -> next;
    ref2 -> next = tmp;

    if(ref1 -> next != NULL) ref1 -> next -> prev = ref1;
    if(ref2 -> next != NULL) ref2 -> next -> prev = ref2;
    ...
    if(ref1 -> prev != NULL) ref1 -> prev -> next = ref1;
    if(ref2 -> prev != NULL) ref2 -> prev -> next = ref2;
...
struct node* concat(struct node* ref1, struct node* ref2) {
    /* Your code here */
    struct node* ref1_tl = end(ref1);
    struct node* ref2_hd = begin(ref2);
    ref1_tl -> next = ref2_hd;
    ref2_hd -> prev = ref1_tl;
...
int distance(struct node* ref1, struct node* ref2) {
    ...
    struct node* tmp = ref1;
    int dist = 0;

    if (ref1 == NULL || ref2 == NULL || ref1 == ref2) return dist;

    while((tmp = next(tmp)) != NULL){
      dist++;
      if (tmp == ref2) return dist;
    }

    tmp = ref1;
    dist = 0;

    while((tmp = prev(tmp)) != NULL){
      dist--;
      if(tmp == ref2) return dist;
    }
...
struct node* unique(struct node* ref) {
    ...
    while((head = next(head)) != NULL)
      if(head -> prev -> data == head -> data) erase (head -> prev);
...
bool includes(struct node* ref1, struct node* ref2) {
    /* Your code here */
    struct node* head1 = begin(ref1);
    struct node* head2 = begin(ref2);
    struct node* tmp = head2;

    while((head1 = next(head1)) != NULL){
