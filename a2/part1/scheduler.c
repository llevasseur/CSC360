...
bool checkTasks() {
    for(int index = 1; index < num_tasks; index++)
    {
...
void scheduler() {
    ...
    while(checkTasks())
    {
        if(tasks[index].state == 5)
        {
            character_code = getch();
            if (character_code != ERR)
            {
                tasks[index].state = 0;
            }
        }
        if(index == current_task && tasks[index].state == 1)
        {
            return;
        }
        if(tasks[index].state == 0)
        {
            if(current_task == index)
            {
                return;
            }
        ...
        }else if(tasks[index].state == 4 && tasks[index].wake_time <= time(0))
        {
        ...
        }else if(tasks[index].state == 2 && tasks[tasks[index].wait_for].state == 3) // wait for task to complete
        {
            int tmp = current_task;
            current_task = index;
            tasks[index].state = 1;
...
void task_wait(task_t handle) {
    ...
    //if (tasks[handle].state == 1) return;
...
void task_sleep(size_t ms) {
    ...
    //time_t now = time(0);
    tasks[current_task].wake_time = time(0) + ms/1000;
